...
from .forms import VkApiAutopostForm, TgApiAutopostForm, DebugVKPostForm
from .models import *
...
def autopost_tokens_view(request):
    if request.method == 'POST':
        ...
        if vk_form.is_valid() and tg_form.is_valid():
            # Получите текущие токены и ID группы из базы данных
            vk_token_instance = VkApiAutopost.objects.first()
            tg_token_instance = TgApiAutopost.objects.first()

            # Если экземпляры токенов существуют, обновите их, иначе создайте новые
    ...
    context = {
        'vk_form': vk_form,
        'tg_form': tg_form,
        'vk_token_instance': vk_token_instance,  # Добавьте этот параметр
        'tg_token_instance': tg_token_instance,  # Добавьте этот параметр
    }
...
class DebugTGAPIView:
    ...
    def get_tg_token(self):
        try:
            tg_token_instance = VkApiAutopost.objects.first()
            return tg_token_instance.VkTokenAutopost, tg_token_instance.VkGroupId
        except VkApiAutopost.DoesNotExist:
            return None, None
...
def debug_tg_post_view(request):
    ...
    if request.method == 'POST':
        form = TgApiAutopostForm(request.POST)
